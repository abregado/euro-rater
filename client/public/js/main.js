        ...
        if (raw) {
            ...
            return {
                orderedEntries: (parsed.orderedEntries ?? [])
                    .filter((e) => validIds.has(e.countryId))
                    .map((e) => ({
...
function getDisplayEntries() {
    if (!dragState)
        return appState.orderedEntries;
    const ds = dragState;
...
function renderList() {
    ...
    entries.forEach((entry) => {
        ...
        li.addEventListener('click', (e) => {
            ...
            if (target.closest('.drag-handle') || target.closest('.emoji-chip'))
                return;
...
// --- Emoji Picker ---
function openEmojiPicker(countryId) {
...
function makeEmojiButton(emoji, entry) {
...
function renderEmojiPickerContent() {
    ...
    const dialog = document.getElementById('emoji-picker-dialog');
    const savedScroll = dialog.querySelector('.emoji-picker-body')?.scrollTop ?? 0;
    dialog.innerHTML = '';
...
// --- Drag ---
function startDrag(e, countryId) {
    e.preventDefault();
...
function onDragMove(e) {
    if (!dragState || e.pointerId !== dragState.pointerId)
        return;
    e.preventDefault();
    const rows = Array.from(document.querySelectorAll('#country-list > li:not(.is-dragging)'));
...
function initSettings() {
    ...
    const modeCheckbox = document.getElementById('settings-server-mode');
...
function handleImport(file) {
    ...
    reader.onload = (e) => {
        try {
            ...
            if (!Array.isArray(data.orderedEntries))
                return;
            const validIds = new Set(COUNTRIES.map(c => c.id));
